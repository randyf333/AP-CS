...
class DocumentIndex extends TreeMap<String, TreeSet<Integer>>
{
   ...
   */
   Map<String,TreeSet<Integer>> index;
   ...
   */
   public void addWord(String word, int lineNum)
   {
      ...
      if(index.keySet().contains(upper))
      {
         TreeSet set = index.get(upper);
         set.add(lineNum);
         index.put(upper,set);  
      }else{
   ...
   public String toString()
   {
      ...
      {
         String nums = "";
         Iterator<Integer> i = index.get(s).iterator(); 
         for(int x = 0; x < index.get(s).size()-1; x++)
         {
            nums += i.next() + ", ";
         }
         nums += i.next();
